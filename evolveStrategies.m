...
%     end
    ...
    for i=1:size(worstStratsI, 2)
        ...
        % if there are more worstStrats guys than there are topHalfStrats,
        % extend permutation of topHalfStrats
        if length(topHalfStratsI)/length(worstStratsI) < 2
            randTopHalfI = [randTopHalfI, randperm(length(find(topHalfStratsI(:,i)>0)))];
            if length(topHalfStratsI)/length(worstStratsI) < 1
                randTopHalfI = [randTopHalfI, randperm(length(find(topHalfStratsI(:,i)>0)))];
                if length(topHalfStratsI)/length(worstStratsI) < 0.5
                    randTopHalfI = [randTopHalfI, randperm(length(find(topHalfStratsI(:,i)>0)))];
                end %if
            end %if
        end %if
